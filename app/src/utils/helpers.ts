...
export function useAsync<ResultType, ErrorType>(
  ...
  const [state, setState] = useState<AsyncProgress<ResultType, ErrorType | Error>>({ kind: 'running' });
    ...
    () => {
      try {
        promiseProducer()
          ...
          .catch((reason: ErrorType) => setState({ kind: 'failure', reason }));
        if (state.kind !== 'running') setState({ kind: 'running' });
      } catch (err) {
